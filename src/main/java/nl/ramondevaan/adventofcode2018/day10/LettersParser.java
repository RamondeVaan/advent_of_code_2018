...
import nl.ramondevaan.adventofcode2018.util.Letter;
...
public class LettersParser implements Parser<boolean[][], String> {
  ...
  public String parse(final boolean[][] toParse) {
    ...
    for (final var gap : gaps) {
      final var size = gap - from;
      if (size != 0) {
        builder.append(parseLetter(toParse, from, size).letter());
      }
      from = gap + 1;
    }
    builder.append(parseLetter(toParse, from, columns - from).letter());
